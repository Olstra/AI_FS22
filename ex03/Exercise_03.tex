...
\begin{exercise}[Heuristics]{1}
        \begin{subexercises}
            ...
            \item The heuristic is admissible as it can't overestimate the cost to reach the goal. To move an object in the real problem the farmer has to at least make one turn or two if he is on the wrong shore. As we are counting these actions but without regard for restriction 2. and 3. which both introduce additional cost by requiring that we bringing some objects back to the starting shore we are guaranteed to be less or at most equal to the optimal cost.
            
            
            \item The heuristic is consistent, this is because $c(n,a,n') = 1$ and $h(n')$ can only decrease by at most $1$ this is because we are counting the minimal number of moves the farmer has to make. So $c(n,a,n') + h(n')$ must be at least as large as $h(n)$.
